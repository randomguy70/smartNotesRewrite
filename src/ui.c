...
bool inputString(char* buffer, uint8_t maxLength, bool restrictFirstChar, int boxX, int boxY, int boxWidth)
{
	...
	while(true)
	{
		...
		{
			character = getCharFromKeyPress(textMode, keyPressed, false);
			if(character != '\0')
			{
...
char getCharFromKeyPress(enum textMode mode, uint8_t keyPressed, bool allowNewLines)
{
	static const unsigned char numsDat[] =
	{
		...
		0x0, 0xa , 0x2B, 0x2D, 0x2A, 0x2F, 0x5E, 0x0, 
	...
	{
		0x0, 0x0 , 0x0 , 0x0 , 0x0 , 0x0 , 0x0 , 0x0 , 
		0x0, 0xa , 0x22, 0x57, 0x52, 0x4D, 0x48, 0x0 , 
	...
	{
		0x0, 0x0 , 0x0 , 0x0 , 0x0 , 0x0 , 0x0 , 0x0 , 
		0x0, 0xa , 0x22, 0x77, 0x72, 0x6d, 0x68, 0x0 , 
	...
	if((keyPressed >= 48) || (!allowNewLines && (keyPressed == 9)))
	{
		return '\0';
	}
